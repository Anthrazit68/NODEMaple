...
NODEXML := module()
	uses NODEFunctions; # , Units[Simple]; https://mapleprimes.com/questions/236887-UnitsSimple-Min--Bug
	description "Funksjoner for import og export av XML filer":
	option package;
	export XMLFileInOut;
	local XMLWrite, XMLImport, XMLMaterialTimber, XMLMaterialConcrete, XMLMaterialSteel, XMLSectionSteel, XMLSectionTimber, XMLCalculations, XMLRead, GetStoreitemsCalculations, runAfterXMLImport, checkBeforeWriteout;
	...
	XMLRead := proc(requesteddata::list, WhateverYouNeed::table)
		...
		if type(afilename, list) = false then
			...
			logfile := FileTools[Text][Open](cat(StringTools:-StringSplit(afilename, ".xml")[1], ".log"), create=true, overwrite=true);
			successful := true
		end if;
		...
		if HasAttribute(xmltree, "source_software") then
			...
			FileTools[Text][WriteLine](logfile, cat("software: ", software));
		end if;
		
		if HasAttribute(xmltree, "version") then
			xmlversion := AttributeValue(xmltree, "version");					# 2020.08
			FileTools[Text][WriteLine](logfile, cat("version: ", xmlversion));
		end if;

		if HasAttribute(xmltree, "created") then
			created := AttributeValue(xmltree, "created");
			FileTools[Text][WriteLine](logfile, cat("created: ", created));
		end if;
			...
			if requesteddata[i][1] = "projectdata" then
						...
						if numelems(GetChildByName(xmlmaterials, rq2)) > 0 then		# find requested material class
							...
							for k from 1 to numelems(GetChildByName(xmlmaterials, rq2)) do		# get material classes (concrete, timber, steel), should just return one value
								...
								for m from 1 to numelems(xmlmaterial) do							# go through materials definition list
									...
									# FileTools[Text][WriteLine](logfile, cat(xmlmaterial[m], ": ", eval(returndata2)));
				...
				if numelems(GetChildByName(xmltree, "calculations")) > 0 then
					...
					if ContentModelCount(xmlcalculations) > 0 then									# there are some calculations already defined
						...
						if numelems(foundcalculationtype) = 0 then			# this type of calculation is not defined yet
				...
				FileTools[Text][WriteLine](logfile, cat("Requested command '", requesteddata[i][1], "' unknown"));
