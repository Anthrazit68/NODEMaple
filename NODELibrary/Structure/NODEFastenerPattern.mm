...
NODEFastenerPattern := module()
	description "Functions for calculation of fastener connections":
	option package;
	uses NODEFunctions, DocumentTools, Units[Simple];
	
	export CalculateForcesInConnection, SetComponentsCriticalLoadcase, SetVisibilityFastenerPattern, ModifyFastenerPattern, PlotResults, BeamsideForceDirection;

	local GetPointlist, PointlistByGrid, PointlistByCircle, PointlistByText, PointlistRotate, PointlistGetCenter, EccentricMoment, ForcesInPoint, PointlistGeta12, 
			GetNumberOfFastenerDefinitions, maxFIndexFastener, PointlistRemoveDuplicates, calculate_a, BlockShearPath, CheckPointInPolygon;
