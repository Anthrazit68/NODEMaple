# EC5_8_minimumdistance.mm : Eurocode 5 chapter 8 minimum distance
...
PrintMinimumdistance := proc(part, WhateverYouNeed::table)
	...
	for j in variables do
		...
		if ComponentExists(cat("TextArea_", varmin)) and assigned(distance[varmin]) then
			SetProperty(cat("TextArea_", varmin), 'value', round(convert(distance[varmin], 'unit_free')));		# no need for unit check
		end if;
		if ComponentExists(cat("TextArea_", varmin_max)) and assigned(distance[varmin_max]) then
			SetProperty(cat("TextArea_", varmin_max), 'value', round(convert(distance[varmin_max], 'unit_free')));	# no need for unit check
		end if;
